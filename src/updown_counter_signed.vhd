library ieee;
use ieee.std_logic_1164.ALL;
use ieee.numeric_std.all;

entity updown_counter_signed is
	generic(
		R : integer := 7
	);
	port(
		clk : in std_logic;
		rst : in std_logic;
		enable : in std_logic;
		cnt : out std_logic_vector(R-1 downto 0)
		);
end updown_counter_signed;

architecture src of updown_counter_signed is
	
begin

	up_down_counter : process(clk, rst)
            -- constant WR : integer := 2**R - 1;
			constant counter_min : signed(R-1 downto 0) := (R-1 => '1', others => '0');
			constant counter_max : signed(R-1 downto 0) := (R-1 => '0', others => '1');
			constant counter_step : signed(R-1 downto 0) := (0 => '1', others => '0');
            variable counter_updown : std_logic := '0';
            variable counter : signed(R-1 downto 0) := counter_min;
		begin
			if rst = '0' then
				counter := (others => '0');
				counter_updown := '1';
			elsif rising_edge(clk) and enable = '1' then 
				if counter_updown = '1' and counter < counter_max then
					counter := counter + counter_step;
				elsif counter_updown = '0' and counter > counter_min then
					counter := counter - counter_step;
				elsif counter = counter_min then
					counter_updown := '1';
					counter := counter + counter_step;
				elsif counter = counter_max then
					counter_updown := '0';
					counter := counter - counter_step;
				end if;
			end if;

            cnt <= std_logic_vector(counter(R-1 downto 0));

	end process;

end src;